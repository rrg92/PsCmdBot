<#
	HTTP BASE functions
		...
		Estas são as funcoes HTTP base que usaremos.  
...
Function Start-HttpRequest {
	...
	if($data -and 'POST','PATCH','PUT' -Contains $method){
		...
		if($contentType -eq "multipart/form-data"){
			...
			#Itera...
			$BodyLines = @()
			#Vamos usar um boundary com esse formato: PowershellCustomHttpLib_GUID.
			$boundary = "PowershellCustomHttpLib_" + [System.Guid]::NewGuid().ToString();
	...
	try {
		...
		if(!$HttpRequest.WebResponse){
			...
			try {
				...
				$PassError | Add-Member -Force Noteproperty HttpErrorDetails $ErrorDetails
	...
	try {
		...
		while($true){
			...
			if($HttpRequest.WebResponse.StatusCode -in (308,302) -and $MaxRedirects -ne $null){
				...
				$e = New-Object System.Exception("HTTP_REDIRECT:Maximum redirects reached $MaxRedirects")
				$e | Add-Member -Force Noteproperty req $HttpRequest
				$e | Add-Member -Force Noteproperty status  $HttpRequest.WebResponse.StatusCode
				$e | Add-Member -Force Noteproperty max $MaxRedirects
